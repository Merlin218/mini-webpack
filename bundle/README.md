...
三个过程：
1. 构建webpack_modules数组
   - 从入口文件出发，需要对将源码解析成AST树
   - 对AST树进行转化
     - 一是找到该文件的所有外部依赖项，收集到deps中
     - 二是对源码中`require('xxx')`修改为`require(moduleId)`，可以实现通过webpack_require来获取模块。
   - 返回该模块的信息，最后生成整个项目的模块树
   - 再将模块树扁平化，生成webpack_modules数组
2. 编写webpack_require函数，用于内部加载模块，内部根据moduleId进行加载
   - 先会从`cache_webpack_modules`中获取加载过的模块
   - 没有的话再到webpack_modules中回去模块加载，进行缓存，最后返回
3. 运行入口函数：`webpack_require(0)`
